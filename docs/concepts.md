...
> **Enforcement scope (#715).** `supervised` is the daemon-enforced approval boundary for the realistic threat — a prompt-injected agent. A manipulated LLM can only *emit* tool-call requests; Pi routes every one to agent-tempo's tool handler, which engages the gate (non-`low-risk`) or hard-blocks (exec tools). The agent cannot skip the gate — it doesn't control the hook. The daemon also derives `failMode` from the durable policy (falling `closed` on any lookup failure — no-fail-open), so an engaging agent can't self-downgrade a supervised player out of fail-closed. The **residual** is *process compromise*: code execution inside the Pi process (host RCE bypassing the handler). That requires OS-level process sandboxing, tracked as a separate future `'sandboxed'` posture (**#724**) — it is not a gap in `supervised`'s intended scope. `supervised` targets prompt-injection and is a real enforcement boundary against that threat.
