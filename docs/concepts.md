...
> **Enforcement scope (#712).** The daemon enforces fail-closed for a `supervised` player's gate **engagements**: the gate cross-checks the durable `guardrailPolicy` (daemon-read, populated at spawn) and derives the `failMode` itself, so the value an engaging agent stamps on its `gate_pending` frame can't self-downgrade a supervised player out of fail-closed (an `open` claim against a `supervised` policy is overridden to `closed` and a `failmode-override` audit record is written). This is **not a hard boundary** against an agent that *skips* gate-engagement (runs a tool without emitting `gate_pending`) or bypasses the client-side MD-C exec-block — a true boundary requires **spawn-time tool-restriction** (the agent physically lacks the tools), tracked separately as **#715**. *Post-restart window:* on daemon restart the in-memory ingest tokens are invalidated, so existing players' gate engagements are rejected (403) until a re-spawn re-mints; in that window a `supervised` player's gate-client still fail-closes on its own derived deadline (client-side safety holds), but the gate is not daemon-mediated — the #715 client-cooperative residual.
