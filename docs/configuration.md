...
| `AGENT_TEMPO_TASK_QUEUE` | `agent-tempo` | Task queue name |
| `AGENT_TEMPO_ENSEMBLE` | `default` | Ensemble name |
| `AGENT_TEMPO_CONDUCTOR` | `false` | Enable conductor mode |
| `AGENT_TEMPO_PLAYER_NAME` | *(random hex)* | Player name on startup |
| `AGENT_TEMPO_DEFAULT_AGENT` | `claude` | Default agent type (`claude` or `copilot`) |
| `AGENT_TEMPO_CLAUDE_BIN` | *(auto-detected)* | Path to a custom `claude` executable. Takes precedence over the config file setting and `which`/`where` auto-detection. Useful when multiple Claude versions are installed or the binary is not on `PATH`. |
| `AGENT_TEMPO_DEV_MODE` | `false` | Enable dev profile (`1` or `true`). Flips home dir to `~/.agent-tempo-dev/`, HTTP port to 8474, Temporal namespace to `agent-tempo-dev`, task queue to `agent-tempo-dev`. Also enables the mock adapter (`agent: 'mock'`). Same effect as the `--dev` CLI flag. |
...
**Legacy `httpToken` — removed in 2.0 (#794):**

> ⚠ **2.0 no longer adopts `config.json#httpToken`.** The 1.x daemon adopted `httpToken` as
> the T1 read token on first boot; the 2.0 daemon does not. If you have `httpToken` in your
> config, it is ignored — the daemon auto-generates a fresh `readToken` instead. Any client or
> reverse proxy configured with the old `httpToken` value will receive `401`. Read the
> auto-generated `readToken` from `~/.agent-tempo/config.json` or supply
> `AGENT_TEMPO_HTTP_READ_TOKEN` explicitly before upgrading.
...
**Dev-mode mock adapter** (requires `AGENT_TEMPO_DEV_MODE=1`):

| Variable | Default | Description |
|----------|---------|-------------|
| `AGENT_TEMPO_MOCK_MODE` | `echo` | Mock adapter mode: `echo`, `scripted`, `silent`, or `chaos`. |
| `AGENT_TEMPO_MOCK_SCENARIO` | *(none)* | Bare scenario name or absolute path to a YAML scenario file. Required when `mockMode: scripted`. |
| `AGENT_TEMPO_MOCK_CHAOS_DELAY_MS` | `0` | Fixed pre-reply delay in ms injected by chaos mode. |
| `AGENT_TEMPO_MOCK_CHAOS_FAIL_RATE` | `0.05` | Per-message probability (0–1) of throwing an error in chaos mode. |
| `AGENT_TEMPO_MOCK_CHAOS_CRASH_RATE` | `0.01` | Per-message probability (0–1) of `process.exit(1)` in chaos mode. |
| `AGENT_TEMPO_MOCK_CHAOS_SEED` | *(random)* | 32-bit integer seed for the chaos PRNG (mulberry32). Set for reproducible failure injection. |
...
export AGENT_TEMPO_CLAUDE_BIN=/usr/local/bin/claude-nightly

# Or persist in config file
agent-tempo config set claude-bin /usr/local/bin/claude-nightly
```

**Resolution order:** `AGENT_TEMPO_CLAUDE_BIN` env var → config file → `which`/`where` → bare `claude` fallback.
