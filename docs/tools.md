...
These tools are available inside Claude Code sessions connected to agent-tempo.
...
| `recruit` | Spawn a new Claude Code session in a directory. Can recruit a conductor with `conductor: true`. When `host` is set, validates the target daemon is live and supports the requested agent before spawning; pass `force: true` to bypass pre-flight (#274). Dev mode only: `agent: 'mock'` with optional `mockMode` (`echo` \| `scripted` \| `silent` \| `chaos`) and `mockScenario` (bare name or YAML path, required for `scripted`). #131 Phase C — `agent: 'claude-api'` runs headless via the Anthropic Messages API; requires `ANTHROPIC_API_KEY` env var + the `@anthropic-ai/sdk` optional dependency installed. Optional `model` arg overrides the default (`claude-opus-4-7`); falls back to `AGENT_TEMPO_API_MODEL` env. claude-api players have access to agent-tempo MCP tools (cue, report, recall, ensemble, …) but NOT file-edit / shell / web tools — use `agent: 'claude'` for tasks requiring those. #449 Phase C — `agent: 'opencode'` runs headless via [SST OpenCode](https://opencode.ai); requires OpenCode CLI (`npm install -g opencode-ai`) + `@opencode-ai/sdk` optional dependency. Pass `model: 'provider/name'` (e.g. `'anthropic/claude-opus-4-7'`, `'openai/gpt-4o'`, `'ollama/llama3'`). OpenCode has MCP-native tool access and persists session history server-side across restarts. #520 — `agent: 'claude-code-headless'` runs the official Claude Code CLI as a per-turn `claude -p --output-format stream-json` subprocess; turns bill against the host's existing Claude Code subscription extra-usage credits (Pro / Max plans) — the only ToS-clean way for a third-party tool to tap that pool. Requires `claude` binary on PATH AND a logged-in Claude Code session (`claude auth login`); pre-flight rejects with an actionable error otherwise. Optional `permissionMode` recruit knob (`acceptEdits` \| `auto` \| `bypassPermissions` \| `default` \| `dontAsk` \| `plan`; default `acceptEdits`) OR `dangerouslySkipPermissions: true` (mutually exclusive). Headless players have full Claude Code tool access (Bash, Read, Write, Edit, Glob, Grep, WebSearch, WebFetch). #632 Phase 3a — `agent: 'pi'` runs headless via the [Pi AI](https://github.com/badlogic/pi-mono) SDK; requires `pi-ai` optional dependency (`npm install -g pi-ai`) on Node 22.19+. Optional `model: 'provider/model'` (#734) — built-in providers (e.g. `'anthropic/claude-opus-4-7'`, `'github-copilot/gpt-4o'`) and custom `~/.pi/agent/models.json` providers (e.g. `'lmstudio/qwen/qwen3.6-27b'`, `'ollama/llama3'`) both resolve; falls back to the `AGENT_TEMPO_PI_MODEL` env var; absent → Pi's own default. Enables hybrid ensembles: a frontier-model conductor recruiting local-model workers. Pi players run the FULL tool surface — Pi built-ins (incl. shell) plus the agent-tempo MCP tools — with no permission layer, like the other adapters (the former `toolAccess`/`guardrailPolicy` knobs were removed); the `noExtensions: true` flag still blocks third-party disk/package extensions from loading into a recruited player (supply-chain hygiene). |
| `report` | Send updates to the conductor. No-op if no conductor exists. |
| `respond` | **#700 P2.** Answer a correlated `[Q <questionId>]` question from the command-center planner. Pass the `questionId` from the cue tag + your answer `text`; it's parked on the maestro Q&A mailbox (TTL 1h) for the inbox-less planner to read back. `from` is your player id (no spoofable arg). Use `cue` for normal player-to-player messages; `respond` only for tagged planner questions. |
| `schedule` | **#793 canonical.** Schedule messages to a player. `action` = `create` (default) \| `cancel` \| `list`. `create` (omit `action` for back-compat) makes a one-shot/recurring/delay/cron schedule (`name` + `message` + `target` + one timing of `at`/`delay`/`every`/`cron`); `cancel` removes a named schedule; `list` shows all active schedules. |
| `unschedule` | DEPRECATED alias of `schedule` action="cancel" — kept for back-compat (forwards with the same schema). Cancel a named schedule. |
| `schedules` | DEPRECATED alias of `schedule` action="list" — kept for back-compat. List all active schedules. |
...
| `recall` | Read your own message history. Shows received messages by default; pass `includeSent: true` for the full timeline. `limit` caps results (default 20, max 100); `offset` pages the timeline (gh-style `Showing X-Y of Z messages. Use offset: N for next page.`); `previewLength` truncates bodies to N chars (unset = full text). #128 unified the output with the TUI `/recall` and CLI `agent-tempo recall` via a shared formatter. |
| `state` | **#793 canonical.** Per-player saveable state slots. `action` = `save` \| `fetch` \| `clear` (required). `save` writes a curated slot (`content` + optional `key`, owner-only); `fetch` reads self or a peer (`key`/`playerId`, defaults to own `"main"`); `clear` empties a slot (owner-only, idempotent). Max 4 slots × 32 KiB; survives `destroy`. |
| `save_state` | DEPRECATED alias of `state` action="save" — kept for back-compat (forwards with the same schema). **#334.** Write a curated artifact to a named slot (max 4 slots × 32 KiB). Owner-only. |
| `fetch_state` | DEPRECATED alias of `state` action="fetch" — kept for back-compat. **#334.** Read a saved-state slot for yourself or a peer; defaults to own `"main"` slot. Works even after `destroy`. |
| `clear_state` | DEPRECATED alias of `state` action="clear" — kept for back-compat. **#334.** Clear one of your own saved-state slots. Owner-only, idempotent. |
| `coat_check` | **#793 canonical.** Ensemble-shared transient store. `action` = `put` \| `get` \| `list` \| `evict` (required). `put` stashes content (`summary` + `content`[, `contentType`, `ttlMs`]) → ticket; `get` redeems a `ticket` (bumps fetch audit, Update); `list` surveys headers (`putBy`/`prefix`/`unfetchedOnly`); `evict` removes a `ticket` early (owner-or-conductor). Max 32 KiB/entry, 20 slots, TTL [1h, 30d] default 7d. |
| `coat_check_put` | DEPRECATED alias of `coat_check` action="put" — kept for back-compat. **#318 (ADR 0008).** Stash a large content body; returns a ticket. Pair with `cue`'s `attachmentTicket`. |
| `coat_check_get` | DEPRECATED alias of `coat_check` action="get" — kept for back-compat. **#318 (ADR 0008).** Redeem a ticket and pull the body; `null` on miss. Bumps fetch audit (Update). |
| `coat_check_list` | DEPRECATED alias of `coat_check` action="list" — kept for back-compat. **#318 (ADR 0008).** List entry headers (body omitted), read-only. Filters: `putBy`/`prefix`/`unfetchedOnly`. |
| `coat_check_evict` | DEPRECATED alias of `coat_check` action="evict" — kept for back-compat. **#318 (ADR 0008).** Evict an entry before TTL. Owner-or-conductor gate; no throw on miss. |
| `hosts` | **#274.** List all daemons polling this Temporal namespace, joined with their boot-signaled capability profile (default agent, available player types, platform, claude bin basename). Optional `includeStale: true` shows hosts not seen in the last minute; `force: true` bypasses the 3-second result cache. Output matches CLI `agent-tempo hosts` and TUI `/hosts` (shared formatter, AC10a). |
...
| `gate` | **#793 canonical (partial merge).** Define and inspect quality gates. `action` = `define` \| `list` (required). `define` sets/replaces a gate (`task` + `criteria`); `list` shows gates and per-criterion status (optional `task`/`status` filters). Conductor only. To record pass/fail use the separate `evaluate_gate` tool (not folded in — it's a runtime op, not gate-definition CRUD). |
| `quality_gate` | DEPRECATED alias of `gate` action="define" — kept for back-compat (forwards with the same schema). Define or replace a quality gate. Conductor only. |
| `evaluate_gate` | Mark one or more criteria on a quality gate as passed or failed. Conductor only. **Stays a separate tool** (not merged into `gate`) — it's a runtime evaluation operation, not gate-definition CRUD. |
| `gates` | DEPRECATED alias of `gate` action="list" — kept for back-compat. List quality gates and their status. Conductor only. |
| `stage` | **#793 canonical.** Track a fan-out/fan-in pipeline stage. `action` = `create` (default) \| `list` \| `cancel`. `create` (omit `action` for back-compat) opens a stage (`name` + `players`[, `failurePolicy`]); `list` shows stages and per-player report status; `cancel` cancels a named stage. Conductor only. |
| `stages` | DEPRECATED alias of `stage` action="list" — kept for back-compat. List stages and their status. Conductor only. |
| `cancel_stage` | DEPRECATED alias of `stage` action="cancel" — kept for back-compat. Cancel an active stage by name. Conductor only. |
...
| `reset` | **Phase 3d.** Clean-wipe a headless Pi player's conversation context — the target starts a FRESH session (no transcript replay). Params: `playerId` (required), `reason` (optional, max 500 chars). Any player can reset a peer; the conductor is not required. Wire protocol: `setPendingReset` signal → `pendingReset` query polled by the player's `ResetPump` → `ackReset` signal on completion. |

## Version History

### 2.0 beta.2 Changes (#793)

- **Tool-family merge (additive, non-breaking).** Five canonical multi-action tools land — `coat_check` (put/get/list/evict), `state` (save/fetch/clear), `schedule` (create/cancel/list), `stage` (create/list/cancel), `gate` (define/list). Each takes an `action` enum. Net-new names (`coat_check`, `state`, `gate`) require `action`; reused names (`schedule`, `stage`) default `action` to `create` so existing callers that omit it are unaffected. (#793)
- **Old per-action names kept as forwarding aliases.** `coat_check_*`, `*_state`, `unschedule`, `schedules`, `stages`, `cancel_stage`, `quality_gate`, `gates` all stay registered with their exact original schemas (alias-not-remove invariant). Alias removal is a future GA event. (#793)
- **`evaluate_gate` unchanged and still separate** — a runtime evaluation op, not gate-definition CRUD (partial-merge boundary). (#793)
- **Surface-drift gate hardened** — `scripts/check-surface-drift.js` now enumerates tool names from the real `buildAllTempoTools()` output (via `scripts/enumerate-tool-names.ts`) and fails loud on a zero count, so new canonicals/aliases can't silently escape the drift check. (#793 §6 / #707)
...
> **Breaking change in v0.25.0-beta.1**: The wire protocol between sessions and workers changed. If you are upgrading from v0.24.x, run `agent-tempo down` and `agent-tempo up` to reinitialize. Sessions from different versions cannot interoperate.
