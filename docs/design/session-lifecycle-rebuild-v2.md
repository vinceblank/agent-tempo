...
| `WorkflowNotFound` (SDK-native) | Any operation against a pinned runId that doesn't exist | Temporal SDK default | **Adapters: normative §9.4 — stop heartbeat, stop delivery, shutdown, exit. No retry, no re-resolve by workflowId alone, no `workflow.start`.** Operator tools (`detach`, `destroy`, `attachmentInfo`, etc.): surface `Error: no workflow "${name}" at the expected run. It may have been destroyed; run \`recruit\` to create a fresh one.` and exit with code 2. Tools MUST NOT re-resolve without the runId and MUST NOT retry with a reuse/conflict policy — that is the #102 mechanism pre-rebuild and is forbidden everywhere. |
