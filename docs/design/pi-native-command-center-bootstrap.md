...
- E: `pi-ui.ts` add `registerTool` to the McExtensionAPI slice (verify vs Pi 0.78 types; pi-drift gate). `actions.ts`: `ask({target,question,questionId})` + `readAnswer(questionId)`. `extension.ts`: register Pi TOOLS (`ask`, `handoff`, `cue`, `recruit`, `observe_board` → returns the **FULL board** as text — rows width-truncated + inner-tail capped, **NO player-row-count cap**: completeness is intended for this planner read tool, and a `+M more` cap would blind the planner to the capped players. The result is bounded by line-width × player-count — small for realistic ensembles, and even a pathological 100-player case is ~100 short lines, not a token blowout) alongside the human commands. **`ask` tool = YIELD-NOT-POLL** (POST + return "dispatched; you'll be woken"; SSE wake resumes the turn). Human `/ask` (cmdAsk) polls `readAnswer` **with a BOUNDED timeout + cancel** (don't wedge the TUI).
...
- Tests `test/` (mocha): ask routes+yields; observe_board returns the full board text; handoff cue/coat-check; both tool+command land on actions.

**Commit 5 — [GA] G: unified guardrails** ★ second metadata touch (policy-storage) + bundle + wire
- **Policy-storage [TA-ruled]:** `guardrailPolicy` in **durable `SessionMetadata`** (NOT the env-path `toolAccess` uses) — values **`autonomous` (default) | `monitored` | `supervised` | `observe-only`**. Additive; recruit-arg → metadata-set → persisted; **arm-at-boot reads it on attach**. **Durable specifically so a restart re-reads the persisted policy at arm-at-boot on every attach — a previously-`supervised` agent stays `supervised`, never silently re-arming `autonomous`. WHY durable (not the `toolAccess` env-path): a policy delivered only via spawn-env that a restart/re-attach failed to re-thread would silently downgrade a supervised agent to autonomous — that downgrade IS the guardrail bypass. Durable `SessionMetadata` PREVENTS it: the policy persists across restart/migrate/re-attach, so arm-at-boot always reads the real posture.** Additive wire → `WIRE-PROTOCOL.md` + `npm run build` (the SECOND bundle-rebuild touch). *(tempo-architect ruling — fidelity-confirmed.)*
