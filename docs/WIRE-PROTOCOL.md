...
| `bgFullUuid` | `string?` | Full session UUID generated by the spawn activity and passed to `claude --bg --session-id <uuid>`. Populated only when `spawnMode === 'bg'`. Stable across restart so the supervisor adopts the same slot. |
| `bgShortId` | `string?` | Supervisor's 8-char short id, deterministically `bgFullUuid.slice(0, 8)`. Populated only when `spawnMode === 'bg'`. Used by `destroy` to invoke `claude stop <shortId>`. |
