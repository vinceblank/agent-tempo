...
| `spawnMode` | `'terminal' \| 'bg' ?` | #596 / ADR 0016. Experimental spawn-mode override for the interactive `claude-code` adapter. `'terminal'` (default when absent) opens a per-recruit terminal window; `'bg'` routes through `claude --bg` so Anthropic's per-user supervisor takes pty ownership (visible in `claude agents`). Additive — old workflows that omit the field land on the `'terminal'` branch unchanged. Silently ignored when `agent !== 'claude'`. See [docs/ops/bg-spawn.md](ops/bg-spawn.md). |

### `SpawnOutboxEntry` (selected fields)

| Field | Type | Description |
|-------|------|-------------|
| `spawnMode` | `'terminal' \| 'bg' ?` | #596 / ADR 0016. Carries the original recruit's spawn-mode override across restart so the re-spawn lands on the same supervisor (`'bg'`) or terminal-window (`'terminal'`) path. Read from durable `SessionMetadata.spawnMode` by `deliverRestart`. Additive; absent payloads default to `'terminal'`. |

### `SessionMetadata` (selected fields — ADR 0016)

| Field | Type | Description |
|-------|------|-------------|
| `spawnMode` | `'terminal' \| 'bg' ?` | Spawn mode resolved at recruit. Absent on pre-#596 sessions and on non-claude-code adapters. Read by `destroy` (to choose `claude stop` vs `hard-terminate`) and by `restart` (to populate `SpawnOutboxEntry.spawnMode`). |
| `bgFullUuid` | `string?` | **Unused under stdout discovery (ADR 0016 Q1 erratum)** — kept on the metadata shape for forward compat. Originally specced as the supervisor's full UUID pre-assigned via `--session-id`, but `claude --bg` empirically ignores `--session-id` on `claude 2.1.140` and assigns its own UUID. The supervisor's full UUID lives in `~/.claude/daemon/roster.json`; no agent-tempo code currently needs it. |
| `bgShortId` | `string?` | Supervisor's 8-char short id, parsed from the spawn's stdout (`backgrounded · <shortId> (idle …)`) by `spawnClaudeBg` and persisted via `updateMetadataSignal` post-spawn. Populated only when `spawnMode === 'bg'` AND the stdout parse succeeded. Used by `destroy` to invoke `claude stop <shortId>`. |
