...
| `setPendingReset` | `{ resetId: string; fresh: boolean; reason?: string; requestedBy?: string }` | **D14.** Sets a pending context-reset flag the Pi extension polls (`pendingReset` query). Single-slot, latest-wins; the workflow stamps `requestedAt` deterministically (`workflow.now()`). Sent by the `deliverReset` outbox activity (enqueued by the `reset` tool). The flag is carried across `continueAsNew` until acked. |
...
| `getMetadata` | `SessionMetadata` | Returns the full session metadata object (ensemble, playerId, hostname, status, git info, player type, etc.). The `guardrailPolicy?` optional field (added in #700 P2/G as non-breaking additive, only ever shipped in the 1.7.0 betas) was REMOVED with the Pi permission layers (gate removal, 2026-06) — field-level removal of optional data, no query name change; old workflows whose persisted metadata still carries the field are fine (new code ignores it). |
...
| `pendingReset` | `PendingReset \| null` | **D14.** Returns the pending context-reset (`{ resetId, fresh, reason?, requestedBy?, requestedAt }`) the Pi extension should act on, or `null`. Polled like `pendingMessages`; the extension performs the clean-wipe (`newSession`) then clears it via the `ackReset` signal. Single-slot, latest-wins; carried across `continueAsNew` until acked. **Since #750 the Pi pump prefers the combined `pendingIntake` query**; this query remains served for old pumps — deprecation candidate at the next major. |
| `pendingIntake` | `{ messages: Message[]; pendingReset: PendingReset \| null }` | **T0.3 of #747 (#750).** Combined intake for the Pi cue pump: `messages` is identical to `pendingMessages` (undelivered only) and `pendingReset` to the `pendingReset` query, served in ONE billable query per pump tick (was 2). ADDITIVE — both legacy queries remain. New pumps fall back to the legacy pair when this handler is absent (pre-#750 workflows). Acks unchanged: `markDelivered` / `ackReset` (same race-safe id-match semantics). Note `pendingMessages` itself is NOT a deprecation candidate — it stays the primary intake for every non-Pi adapter. |
...
| `AgentTempoAttachmentState` | `Keyword` | **v0.25.** Current attachment phase: `booting \| attached \| processing \| awaiting \| draining \| detached \| gone`. Enables external observers (mission-control, monitoring, daemon reconcile-on-boot) to query session readiness without polling the `attachmentInfo` query. |
