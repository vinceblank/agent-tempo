...
| `getMetadata` | `SessionMetadata` | Returns the full session metadata object (ensemble, playerId, hostname, status, git info, player type, etc.). `guardrailPolicy?` (added in #700 P2/G, non-breaking additive) carries the durable guardrail posture (`autonomous`/`monitored`/`supervised`/`observe-only`) so arm-at-boot reads the real posture on every (re)attach. |
...
| `pendingReset` | `PendingReset \| null` | **D14.** Returns the pending context-reset (`{ resetId, fresh, reason?, requestedBy?, requestedAt }`) the Pi extension should act on, or `null`. Polled like `pendingMessages`; the extension performs the clean-wipe (`newSession`) then clears it via the `ackReset` signal. Single-slot, latest-wins; carried across `continueAsNew` until acked. |
