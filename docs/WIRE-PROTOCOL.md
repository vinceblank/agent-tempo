...
| `checkAndSetStatus` | `{ expectedStatus: string; newStatus: string }` | `boolean` | **Legacy — shimmed through PR-D.** Atomically transitions the session's status from `expectedStatus` to `newStatus`. Still used by `performEncore` activity; removed when `encore` is deleted in PR-D. |
