...
| `playerStateKeys` | `string[]` | **#334.** Returns the names of every populated player-saveable-state slot, sorted alphabetically. Empty array when no slots are saved. Used by `fetch_state list: true` and operator/debugging surfaces to enumerate slots without reading content. |
