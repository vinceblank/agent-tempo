...
| `getWireMeta` | `WireMeta` — `{ runId: string; messaging: { received: number; sent: number; outbox: string }; lease: { expiresAt: number \| null; leaseMs: number \| null }; coarse: { currentTool: string \| null; contextTokens?: number; contextPercent?: number } }` | **Daemon-resilience PR-B.** Combined wire-meta for the snapshot fan-out: returns in ONE query exactly what `getRunId` + `getMessagingState` + `getLeaseState` + `getCoarseActivity` return separately (same field semantics — see each row above). `getPlayerWireMeta` polls this once per player per 750 ms aggregate tick, replacing the four-query fan-out that starved the daemon's single workflow thread (~20 q/s at idle — the 2026-07-13 daemon death; `docs/research/daemon-query-timeout-rca.md`). ADDITIVE — all four standalone queries remain served (stable wire surface; older daemons on other hosts in a mixed-version fleet still poll them). Precedent for merging polled queries: `pendingIntake` (#750). |
