...
interface EnsembleStateV1 {
  ...
  lastEventId: number;       // see §7.2 — pass to /v1/events as Last-Event-ID
...
- `eventId` is a strictly monotonic uint64 per ring buffer. Unique within a single daemon process lifetime; restarts reset to zero (a `gap` event covers the gap — see §7).
...
| `gap` | both | `{ from: number; to: number }` | One-shot — emitted when `Last-Event-ID` predates the ring's oldest id; client MUST re-fetch `/v1/state/:ensemble` then reconnect without `Last-Event-ID` |
| `throttled` | both | `{ droppedSince: number; count: number }` | One-shot when ensemble-wide 50 ev/s ceiling trips (§8) |
...
| `chat.compressed` | per-ensemble | `{ dropped: number; since: number }` | Emitted when chat hits the per-ensemble rate cap |
...
```
       ...
       YES                                       NO
        │                                         │
        ▼                                         ▼
 lastEventId >= ringStart?                emit `snapshot`
        │                                         │
   ┌────┴────┐                                    ▼
