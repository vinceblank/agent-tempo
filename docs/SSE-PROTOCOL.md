...
| `GET` | `/v1/ensembles/:ensemble/attachment-info?playerId=<name>` | `application/json` | **#742.** A player's V2 attachment-lifecycle snapshot (`AttachmentInfo`) — thin shim over `TempoClient.attachmentInfo`. The command-center `/attachment-info` board command renders it via the shared `formatAttachmentInfoForDisplay`. T1 read. `playerId` query param required (absent → `400 missing-field`); unresolvable session → `404`. |
| `GET` | `/v1/ensembles/:ensemble/schedules` | `application/json` | **#742.** Active schedules (`ScheduleEntry[]`) for the ensemble — thin shim over `TempoClient.getSchedules` (empty when no scheduler). The command-center `/schedule [list]` board command renders the entries. T1 read. |
...
| `POST` | `/v1/ensembles/:ensemble/{cue,pause,play,release,recruit,restart,reset,destroy,detach,recall,unschedule}` | `application/json` | Safe-write endpoints (PR-7a of #340; per-player destructive verbs added by `feat/daemon-action-http-endpoints`; `reset` added by H5b/#645; `unschedule` added by #742). See § 11b for full request/response shapes. |
...
Eleven POST routes under `/v1/ensembles/:ensemble/<action>` give the dashboard + command-center board a bidirectional surface. Each handler is a thin shim over the daemon's existing `TempoClient` method (the same client the daemon uses for snapshots) — **zero new Temporal signals/queries/updates**.

### Routes

| Action | Body shape | Success response | Notes |
|---|---|---|---|
| `cue` | `{ to: string, message: string }` | `202 { ok, ensemble, to, delivery, queued, phase?, warning? }` | Routes through `ensureMaestroSession` + `sendAsMaestro` so the chat row shows `role: 'maestro-out'` (matches the dashboard's "you, the operator" semantic). **#822 deliverability** (additive): a bounded (1s), soft-failing phase preflight reports whether the target has a live adapter. `delivery: 'live' \| 'queued'`; `queued: boolean`; on `'queued'` (target `detached`/`gone`) also `phase` + `warning: 'queued-undeliverable (<phase>)'`. **Warn-but-queue** — the cue ALWAYS enqueues (durable; auto-redelivers on re-attach); the fields just let the board render `⚠ queued` vs `✓`. Mirrors the MCP `cue` tool's `UNDELIVERABLE_PHASES = {detached, gone}`, but the operator surface warns rather than hard-fails (deliberate divergence: human operator vs autonomous LLM). |
...
| `reset` | `{ playerId, reason? }` | `202 ResetClientResult + { delivery, queued, phase?, warning? }` (`{ playerId, entryId, delivery, queued, … }`) | Per-player D14 clean-wipe — the target starts a FRESH session (no transcript replay). `playerId` required; `reason` forwarded (surfaced to the wiped session + audited). HTTP counterpart of the `reset` MCP tool: ensures the maestro (like `cue`) then enqueues the existing `'reset'` outbox entry (`invokerPlayerId: 'maestro'`, always `fresh: true`). **#822**: reset funnels through the same maestro outbox, so it carries the same additive deliverability fields as `cue` (a reset of a `detached`/`gone` player queues undelivered → `delivery: 'queued'`). |
...
| `recall` | `{ playerId, full?: boolean }` | `200 { ok, ensemble, playerId, messages: number }` (default) **or** `200 { ok, ensemble, playerId, received, sent }` (`full: true`) | Read-shaped (the underlying `TempoClient.recall` returns the player's `{ received, sent }` timeline) but lives on this route group because the dashboard surfaces it on the same PlayerDetail action row — hence `200` not `202`. **Default** projects `received.length + sent.length` into a single `messages` count so the dashboard's `RecallResult` consumer reads a number. **`full: true`** (#742 — the command-center `/recall` board command) returns the raw `{ received, sent }` arrays so the board can render the full timeline via the shared `buildTimeline`/`formatRecall` helpers. |
| `unschedule` | `{ name }` | `200 { ok, ensemble, name }` | Cancel a named schedule (#742 — the command-center `/unschedule` & `/schedule delete`). Thin shim over `TempoClient.cancelSchedule`. Schedule **create** has no thin-shim client method (the MCP `schedule` tool signals the scheduler workflow directly), so create stays on the MCP tool / CLI. `name` required. |
...
- `name` is **required** on `unschedule`; absent → `400 missing-field` (`field: 'name'`).
