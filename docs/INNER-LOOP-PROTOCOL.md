...
**#715 — enforcement scope of `supervised`.** `supervised` is the daemon-enforced approval boundary for the realistic threat: a prompt-injected agent. A manipulated LLM can only *emit* tool-call requests — Pi routes every one to agent-tempo's `tool_call` handler, which engages the gate (non-`low-risk`) or hard-blocks (exec tools at `toolAccess: 'restricted'`). The agent cannot skip the gate or run a dangerous tool directly — it doesn't control the hook. The `failMode` the engaging agent stamps on its `gate_pending` frame is **advisory only**: `GateRegistry.open()` derives the authoritative `failMode` from the player's durable `guardrailPolicy` (daemon-read; populated at spawn and lazily resolved via a bounded `getMetadataQuery` on a cache-miss, falling **closed** on timeout/error — no-fail-open). Posture: `monitored`/`autonomous` → `open`; `supervised`/`observe-only`/unknown → `closed`. An engaging agent therefore can't self-downgrade a `supervised` player — the daemon overrides `'open'` claims to `closed` and writes a `failmode-override` audit record. **Residual:** *process compromise* — code execution inside the Pi process (host RCE bypassing the handler). No client-side gate defends that; it requires OS-level process sandboxing, tracked as a separate future `'sandboxed'` posture (**#724**). That is **not a gap in `supervised`'s intended scope**: supervised targets prompt-injection, and against that threat it is a real enforcement boundary. *Post-restart:* on daemon restart the in-memory ingest tokens are invalidated; existing players' engagements are 403'd until re-spawn re-mints; a `supervised` player's gate-client still fail-closes on its own derived deadline in that window (same process-compromise residual, not a distinct gap).
