...
> **Erratum 2026-05-16 (tempo-architect)** — §"Decision / Q1" pre-assigned-UUID approach is **empirically broken** on `claude 2.1.140` (confirmed on Windows during the #596 graduation E2E). The supervisor refuses `--session-id` under `--bg` (stderr: `warning: --bg manages the session id; ignoring --session-id (use --resume <id> to continue an existing session)`) and invents its own UUID. The pre-assigned UUID never lands in `roster.json`, breaking the destroy lookup.
>
> **Q1 is replaced with stdout-discovery (formerly listed as Rejected Alternative #6).** The supervisor reliably prints `backgrounded · <8-char-short>` as the first stdout line of `claude --bg`; parse that with `/backgrounded\s*[·•]\s*([0-9a-f]{8})/` and stash `shortId` on workflow metadata. The destroy plan is unchanged — still `claude stop <shortId>` resolved from metadata, still routed to the per-host queue via `AgentTempoHostname` SA. Only the discovery mechanism flips.
>
> The "Q1 — endorse with refinement" section below and Alternative #6 are superseded by the implementation; the ADR body is preserved for historical context. See `docs/ops/bg-spawn.md` for the user-facing description.
...
> **Superseded — see Erratum at the top of this ADR.** Preserved here as the
> original decision history. Implementation lives in `src/spawn.ts`
> (`spawnClaudeBg`) + `src/activities/outbox.ts` (`spawnProcess` branch) +
> `docs/ops/bg-spawn.md`.
...
6. ~~**Stdout-parse the supervisor's `backgrounded · <short>` line**~~ — originally listed as the rejected alternative. **Promoted to chosen mechanism** per Erratum 2026-05-16 above; `--session-id` pre-assignment was empirically broken on `claude 2.1.140`.
