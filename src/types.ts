...
export interface GlobalMaestroInput {
  ...
  /**
   * T0.1 (#748) — cost profile, set by the daemon's ensure path from
   * `Config.costProfile` and carried across CAN. Same semantics as
   * `MaestroInput.costProfile`.
   */
...
export interface MaestroInput {
  ...
  /**
   * T0.1 (#748) — cost profile, set by start sites from
   * `Config.costProfile` and carried across CAN. `'cloud'` switches the
   * refresh loop to the additive `refreshEnsembleStateV2` activity
   * (SA/memo scan + observer presence) with a stretched, presence-gated
   * cadence. Absent / `'local'` keeps the pre-#748 behavior byte-identical
   * — including for every in-flight maestro started before the upgrade
   * (their input lacks the field; CAN successors inherit it, so a
   * pre-#748 maestro stays on the old path until destroy/recreate or
   * its natural 5-min idle exit + re-ensure).
   */
  costProfile?: 'local' | 'cloud';
}
