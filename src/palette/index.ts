 ...
 *
 * Consumed by the dashboard (`dashboard/src/components/chat/`) for its
 * chat-input autocomplete — prefix detection, filtering semantics, and the
 * quote-aware tokenizer — via the `agent-tempo/palette` package subpath
 * (Vite alias `agent-tempo` → `../src`).
 *
 * Originally extracted from the Ink TUI's commands module in #471/#472 so
 * the two surfaces stayed in lockstep; the TUI consumer was deleted in
 * #789, leaving the dashboard as the sole consumer. This module is NOT TUI
 * code — the package-subpath export is load-bearing.
 *
