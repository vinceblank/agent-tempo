...
export function registerCoatCheckEvictTool(
  server: McpServer,
  client: Client,
  config: Config,
  getPlayerId: () => string,
  ...
  defineTool(
    server,
    'coat_check_evict',
    `Evict a coat-check entry (#318) before its TTL expires. Owner-or-conductor only — non-owners (and non-conductors) get a permission error.
