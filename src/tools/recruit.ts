...
export function buildRecruitTool(
  ...
  return {
    ...
    params: {
      ...
      // #773 — allow UPPERCASE (anchor + body): custom-provider model ids
      // (LM Studio etc.) are user-defined and frequently mixed-case, e.g.
      // `lmstudio/Qwen/Qwen3-27B`. Still anchored to a leading alphanumeric and
      // restricted to the safe char set `[A-Za-z0-9-/.:_]` (no shell
      // metacharacters — the value is env-injected opaquely by spawn helpers).
      model: z.string().regex(/^[A-Za-z0-9][A-Za-z0-9-/.:_]*$/).optional()
        .describe('Model id. For "claude-api": bare Anthropic id (e.g. "claude-opus-4-7"). For "opencode": combined "provider/model" (e.g. "anthropic/claude-opus-4-7", "openai/gpt-4o", "ollama/llama3"). For "pi": combined "provider/model" (e.g. "lmstudio/Qwen/Qwen3-27B", "anthropic/claude-opus-4-5", "github-copilot/gpt-4o") — built-in and custom models.json providers both supported; ids may be mixed-case. Falls back to AGENT_TEMPO_PI_MODEL (pi), AGENT_TEMPO_API_MODEL (claude-api), or AGENT_TEMPO_OPENCODE_MODEL (opencode), then a constants-pinned default. Ignored for claude / copilot / mock adapters.'),
