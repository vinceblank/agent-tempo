 ...
 * validation script regex-matches `[agent-tempo:dashboard]` to
...
describe('logEvent format', () => {
  ...
  it('emits `[agent-tempo:dashboard] <action>` with no kvs', () => {
    logEvent('app-mounted');
    expect(infoSpy).toHaveBeenCalledWith('[agent-tempo:dashboard] app-mounted');
  });

  it('emits `[agent-tempo:dashboard] <action> key=value` with kvs', () => {
    logEvent('cue-sent', { target: 'tempo-eng', textLen: 42 });
    expect(infoSpy).toHaveBeenCalledWith(
      '[agent-tempo:dashboard] cue-sent target="tempo-eng" textLen=42',
    );
  });
  ...
  it('JSON.stringifies string values so they keep their quotes', () => {
    ...
    expect(infoSpy).toHaveBeenCalledWith(
      '[agent-tempo:dashboard] error reason="timeout: 30s"',
    );
  });
  ...
  it('routes by level — info, warn, error to their respective console methods', () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[agent-tempo:dashboard] warn-evt x=1');
    expect(errorSpy).toHaveBeenCalledWith('[agent-tempo:dashboard] err-evt y=2');
  });
