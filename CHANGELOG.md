...
- **P1 (#944): `down --help` no longer executes the real `down` (2026-07-15 teardown incident)** — an explicit `--help`/`-h`/`--version`/`-v` was parsed correctly but then CLOBBERED by the positional-verb override in `parseArgs`, so `agent-tempo down --help` ran the destructive verb (stopped the daemon + shared Temporal server, removed .mcp.json) instead of printing help. The clobber affected every verb (`destroy --help`, `up --help`, …) and both argument orders. Help/version flags now win over any positional verb; unknown flags were already rejected with exit 1. Regression-guarded in `tests/cli/parse-args-help-guard.test.ts` (`parseArgs` exported; `cli.ts` `main()` moved behind a `require.main` guard). Also documented in `docs/cli.md`: `down` is whole-HOST teardown (daemon + shared Temporal) even with `--ensemble` scoping — per-ensemble teardown is `destroy` / the MCP tool / the HTTP shutdown surface.

- **P0 (#943): `restore` and `restart` no longer fail with `workflow not found for ID: agent-session-{e}-maestro` (2026-07-15 incident)** — TempoClient's cross-workflow verbs (restart/reset/detach/release/recruit/destroy) host their outbox on the headless maestro session workflow, which was historically created by the TUI (deleted in #789) and later by only SOME daemon HTTP routes — and even then self-reaps after its 24h `workflowExecutionTimeout`. Any ensemble whose maestro session was missing/expired lost BOTH documented recovery paths (CLI `restore`, HTTP/CLI `restart`). Fix: all six maestro-hosted submits now route through `submitOutboxViaMaestro`, which lazily `ensureMaestroSession`s and retries once when the host workflow is gone (`isWorkflowGoneError`); the fast path is unchanged. Regression tests pin the self-heal, the id templates (`agent-session-{e}-maestro` session host + `agent-maestro-{e}` hub), non-gone error propagation, and the fail-loud path when ensure doesn't converge.

- **A fatal worker error no longer kills the daemon (#939, 2026-07-13 incident, PR-D of the daemon-resilience program)** — the daemon's two Temporal workers now run under independent supervisor loops (`src/daemon-worker-supervisor.ts`) instead of a fail-fast `Promise.all` that turned any fatal worker error into a whole-daemon `exit(0)`. Fatal `run()` failure → close the dead worker's NativeConnection, rebuild with capped backoff (1s→30s); HTTP/SSE and the other worker keep serving. Create/connect failures (Temporal unreachable) reconnect indefinitely with a rate-limited beacon and never give up. Restart budget: 5 per rolling 10min (reset after a ≥10min healthy run); exhaustion → loud `[agent-tempo:ALARM]`, `daemon.last-exit.json` marker, **exit 1**. New exit-code contract (`docs/ops/daemon.md`): `0` only for a requested shutdown; boot-guard refusals, drain timeouts, and unhandled fatals also write typed last-exit markers. `/v1/health` gains a `workers` field (per-worker `state`/`restarts`/`lastFatal*` — the heartbeat file refreshes regardless of worker health, so this is the only external truth about dispatch capability; `docs/SSE-PROTOCOL.md` §4.1).
- **`daemon.last-exit.json` crash marker (#934)** — the schema companion to the worker-supervisor above: a one-shot post-mortem marker written BEFORE the process dies (atomic tmp+rename, first-writer-wins so a daemon-authored reason is never clobbered) and surfaced once on the next CLI invocation via `ensureInfra()`. Closed `reason` enum: `worker-give-up`, `boot-guard-refused`, `drain-timeout`, `unhandled-fatal`, `stale-pid-unexplained`. A clean exit (code 0) writes nothing — absence of the file IS the "shut down cleanly" signal. See `docs/design/daemon-last-exit-schema.md`.
- **Query-starvation budgets raised: `isolateExecutionTimeout` 5s→30s + `workflowTaskTimeout` 10s→30s (#931, 2026-07-13 incident, PR-A of the daemon-resilience program)** — the daemon's visibility-driven query load (~20 q/s into one workflow V8 thread) could starve a single sandbox call past the SDK's 5s vm deadline, failing the workflow task, invalidating the sticky cache, and forcing full-history replays that starved the thread further — escalating to a fatal Core error that killed the whole daemon (all ensembles frozen ~14.5h). Worker-side `isolateExecutionTimeout: '30s'` takes effect for everything on daemon restart; `workflowTaskTimeout: '30s'` (new `WORKFLOW_TASK_TIMEOUT` constant, applied at every `workflow.start` site) is a start option — existing runs keep the old value until restart or continueAsNew. Deliberately does NOT flip `reuseV8Context: false` (would trade a starvation outage for an OOM outage — architect ruling §4.6).
- **Session workflow histories now bound replay via early continue-as-new (#940, PR-E of the daemon-resilience program)** — the SDK's default `continueAsNewSuggested` threshold (~10,240 events) effectively never fired; histories grew for days and a full-history replay was a contributing factor in the 2026-07-13 daemon-crash outage. Sessions now trigger continue-as-new at >2,000 events / >2MB, gated behind a `'v2.0-early-can'` patch marker for replay determinism across the version boundary.
- **Combined `getWireMeta` query — 4x snapshot poll-load cut (#937, PR-B of the daemon-resilience program; fast-follow #938)** — one query now returns what `getRunId` + `getMessagingState` + `getLeaseState` + `getCoarseActivity` previously returned as four separate polls, cutting per-player snapshot poll load 4x; also evicts stale #433 in-flight dedup entries on timeout (rss leak fix). #938 fast-follow: on `QueryNotRegisteredError` from a session host running a daemon one release behind (per-host task queues), the client falls back per-field to the pre-#937 four-query fan-out instead of failing outright — plain timeouts still return `null` so a wedged worker isn't re-stormed.
- **`daemon.log` rotation on boot (#930, PR #933)** — `daemon.log` is append-only for the life of the process with no external log shipper; it was observed unrotated at 286.9MB during the 2026-07-13 outage triage. Rotates `daemon.log` → `daemon.log.1` (3-generation retention cap) once at boot, before the append handle opens, whenever the file exceeds 50MB. Rotation failures are logged and swallowed so a stale reader/lock never blocks daemon startup.
...
- **`daemon start --foreground` now actually runs the daemon in the tracked process, on all 3 platforms (#935)** — the flag was never wired to anything: `daemon start` always forked the real daemon as a detached grandchild, so every process-supervisor unit (systemd/launchd/Windows Task Scheduler) tracked a process that exited almost instantly, and a later mid-session crash of the real (untracked) daemon went completely unobserved — the root cause of the 2026-07-13 ~14.5h silent-freeze incident. Linux was the worst case: `Type=simple` + `KillMode=control-group` meant `systemctl stop` tore down the entire cgroup, including the detached grandchild it had just spawned. `daemon start --foreground` now runs the daemon in-process via `runForegroundDaemon()`, with explicit log capture and config propagation (previously free from the OS-owned parent process). `packaging/systemd/agent-tempo.service` switched to `Restart=always` + `RestartSec=5` (a stop is still exempt from any `Restart=` policy); `packaging/launchd/com.agent.tempo.plist` switched `KeepAlive` to the `SuccessfulExit=false` dict form.
- **Windows `daemon install` — three live-found bugs (#941)** — surfaced by the first-ever real `daemon install` + kill-and-recover smoke test against production Task Scheduler (bug-2673/2674/2675 in `.wolf/buglog.json`): (1) `-RepetitionDuration ([TimeSpan]::MaxValue)` threw a Task Scheduler XML validation error that `Register-ScheduledTask`'s non-terminating error handling silently swallowed, leaving `agent-tempo-daemon-recheck` unregistered while the script printed success — fixed with a bounded 10-year duration + `-ErrorAction Stop`/try-catch. (2) `Get-Command agent-tempo` resolved to the `.ps1` shim instead of `.cmd` for an npm-linked install; Task Scheduler cannot launch `.ps1` directly, so the task registered fine and silently never spawned anything — fixed with explicit `.cmd` resolution. (3) A new post-register self-check triggers the task and confirms an actual daemon process appears (up to 30s), correctly branching on already-running-vs-cold-start and on real process liveness rather than PID-file presence (two QA rounds tightened this). See `docs/ops/daemon.md` for the full per-platform reliability writeup, including the still-open Windows gap this smoke test surfaced (native `RestartCount` is unreliable against an externally forced kill).
...
- **`runDaemon()` exit-code seam (#932, PR-0 of the daemon-resilience program)** — factored the daemon's `main()` into an exported `runDaemon(): Promise<0|1>`, the merge boundary #935 (`--foreground`) and #939 (worker supervisor) both build on. No behavior change on its own.
