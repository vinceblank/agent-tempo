...
- **T0.2 idle cue latency** — with SDK idle backoff enabled, the first cue after a period of inactivity can take up to 30s to be noticed by the target. Delivery is durable. Set `AGENT_TEMPO_SDK_POLL_MAX_MS=2000` to restore the legacy 2s fixed cadence. The T1.1 push doorbell (in progress) will eliminate this residual latency.
