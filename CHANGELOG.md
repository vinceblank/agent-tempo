...
- **Headless Claude API adapter (#131 Phase C)** — New third adapter `claude-api` runs entirely
  ...
  (overridable via recruit-arg or `CLAUDE_TEMPO_API_MODEL` env). Per-turn cost telemetry via
  structured stderr log (`[claude-tempo:claude-api] turn-usage …`); wire-protocol signal deferred
  until a consumer lands. Verification-addendum landmines all wired: thinking-block round-trip,
  Opus 4.7 parameter discipline, `input_json_delta` deferred parse, mid-stream error try/catch,
  cache-control breakpoints (last system + last tool, 2/4 used). Design + ADR + research docs
  landed in #339, #344, #452. Implementation spans `src/adapters/claude-api/` + the shared
  `src/server-tools.ts` extraction.
