...
<!-- DRAFT for 1.7.0 — finalizes the 1.7.0 line (beta.0 → beta.8 → stable). Version header and date left blank for operator cut. All content feature-complete. -->

### Cost — Temporal Tier-0 programme (#747)

> **Activation**: optimizations are split between `costProfile: 'local'` (default — byte-identical to pre-1.7.0) and `costProfile: 'cloud'` (opt-in; see Behavioral notes). Set `agent-tempo config set costProfile cloud` and restart the daemon and maestros to activate the cloud-profile gates.

- **Action instrumentation** — per-source Temporal action counters + 24h idle meter tag every outbound Temporal call (`source × kind`) via a `WorkflowClientInterceptor` at all 10 long-lived client sites. Read live at `GET /v1/debug/action-counters` (T1, daemon only) or from `[agent-tempo:action-counters]` log lines (adapter/Pi processes; default 5 min interval, `AGENT_TEMPO_ACTION_LOG_INTERVAL_MS` override). Closes #753. (PR #754)
- **SA diet (T0.5)** — custom Temporal search attributes trimmed 9 → 5, keeping only the five that appear in visibility query expressions. `GitRoot`, `PlayerType`, `IsConductor`, and `Part` migrate to workflow memo (same key names); `AttachmentId` deleted (zero readers). Frees half the 10-Keyword namespace cap, eliminating the recurring "legacy `ClaudeTempo*` SAs block `up`" operator pain point and removing 9 billable SA-upsert sites per session lifecycle. Pre-v1.7 in-flight workflows fall back to legacy SA reads via a dual-read choke point and age out naturally. See `docs/ops/sa-diet-migration.md`. Refs #747. (PR #757)
- **Maestro scan rewrite (T0.1, closes #748)** — cloud profile: maestro session scan switches from cluster-wide list + per-session `getMetadata` fan-out to an SA-filtered ensemble-scoped visibility query; per-player row built entirely from SAs + memo (zero extra actions). Cadence stretches 5s → 20s with live SSE observers, 60s when the daemon has zero subscribers. Local profile: byte-identical to pre-#747 (5s, V1 activity). New `costProfile: 'local' | 'cloud'` config axis — see Behavioral notes. Refs #747. (PR #759)
- **SDK-adapter idle poll backoff (T0.2, closes #749)** — the five SDK-class adapters (copilot, claude-api, opencode, claude-code-headless, mock) stretch their `pendingMessages` poll from 2s toward a 30s cap while idle and snap back to 2s on any delivered message (~15× fewer idle Temporal queries per player: 43,200 → 2,880/day). **Behavioral note: the first cue after a long idle stretch can take up to 30s to surface; in-progress conversations keep 2s responsiveness.** Escape hatch: `AGENT_TEMPO_SDK_POLL_MAX_MS=2000`. Heartbeat/lease cadences unchanged (#249 invariants). Shipped ungated (no costProfile fork). Refs #747. (PR #761)
...
- **`fetchEnsembleChat` presence gate (T0.6 — Tier 0 closer, closes #760)** — cloud-profile maestros skip the ensemble-chat fetch on ticks where zero SSE observers are recorded (the T0.1 presence flag). Chat is a pure observation feed; unwatched ensembles lose nothing, and the first watched tick is a clean catch-up. Removes ≤4 queries + 1 activity-start per unwatched tick, trimming the largest remaining maestro cost line (~63% of the post-T0.4 residual). Determinism: patch marker `v1.8-t06-chat-gate` guards replay-correctness for post-#759 cloud histories. **⚠ Action-counter source tag split** (see Breaking notes): `'maestro'` is now emitted as `'maestro-ensemble'` / `'maestro-global'` / `'maestro-session'`. Closes the Tier-0 cost programme (#747). (PR #774)

### Latency — T1.1 cue doorbell (PRs #776/#805/#803/#804, Refs #747)
...
- **Ghost daemon detection and hygiene (closes #758)** — `daemon stop` now performs a port-ownership sweep unconditionally, including when the cross-profile zombie reaper would otherwise skip (the exact incident path). Kills the port owner only when its command line is a structurally-matched daemon entry; non-daemon squatters get a loud warning and the manual kill command. `daemon start` adds a 10s drain-window pre-flight: if the port is owned, it waits for the prior daemon to finish or refuses with the owning pid and a remedy message. `daemon status` now reports explicit `PHANTOM DAEMON` (pid file ≠ port owner), `GHOST DAEMON` (no pid file, port owned), and `NOT-bound` (tracked pid not serving HTTP) warnings. Port probe via `netstat -ano` (Windows) / `lsof` (POSIX); probe failure degrades to pre-#758 behavior safely. (PR #769)
- **Structural daemon-entry matcher (closes #771)** — replaces path-substring regex with a tokenized command-line parser: executable basename must be `node`/`node.exe`, first non-flag argument must end in `dist/daemon.js`. Closes the false-negative (#771: a local-repo daemon with a quoted Node path was not killed) and the false-positive (a process whose *argument* is a daemon path is no longer matched). Trust-tier system: `pathVerified` flag restricts the zombie reaper, `daemon start` abort, and `daemon status` orphan listing to structurally-verified processes; the port-owner ghost sweep uses structural match alone (port ownership is ground truth). Refs #758. (PR #775)
- **HTTP bind failure retry-with-beacon (closes #768)** — when the daemon's HTTP listener fails to bind at boot (e.g. port squatted by a draining prior daemon), it now enters an indefinite retry loop with capped exponential backoff (1s → 30s steady-state) instead of logging one line and silently running Temporal-only forever. Temporal workers stay up throughout the loop. Three beacon surfaces: (1) `daemon status` names the squatting pid (#769, already shipped); (2) rate-limited log breadcrumb — first failure always logged, then at most one `WARNING: serving no HTTP — bind retry N failed (…)` per 5 minutes; (3) additive `httpDegraded` flag on the host's `HostProfile` — advertised on transitions only (`true` entering degraded, explicit `false` on recovery); rendered as `⚠ HTTP DEGRADED` in `agent-tempo hosts` and `/hosts`. (PR #781)
- **Pid-file liveness re-assertion + port-ownership fallback (closes #811)** — daemon re-writes its pid file after a successful HTTP bind, so a previously stale or absent pid file is self-healed on recovery rather than leaving `isDaemonRunning()` permanently blind. `isDaemonRunning()` now falls back to port-ownership when the pid file is absent, closing the stale-pid strand where recruited sessions could fail to detect a live daemon. Completes the single-host ghost-hygiene arc (#758 / #771 / #801). (PR #814)
- **Workflow registration-order fence + `forceDetach` re-check (G2, closes #797, #798)** — (1) Fence comments at all four signal/update-pair registration sites (S2, S3, S4, C1) with a mandatory-conscious-change note; `tests/conformance/registration-order-fence.test.ts` pins the convention including a KNOWN-EXCEPTION pin for `playerReportSignal`/`setStageSignal` ordering. Silent `fireSchedule` retry-exhaustion promoted to `workflowLog.warn` with schedule + target + cause. (2) `forceDetachUpdate` post-`hardTerminate`-await re-checks the attachment id before any state mutation — a fresh claim landing during the kill-await window no longer gets clobbered. Patch marker `v0.27-force-detach-recheck` evaluated lazily. Zero wire-surface changes. (PR #810)

### New features

- ⭐ **`upgrade-to-2` cutover verb (closes #785)** — `agent-tempo upgrade-to-2 [ensemble]` (flags: `--yes`, `--dry-run`, `--force-drain`) runs the ratified A2 migration protocol for the 1.x → 2.0 transition: `preflight → pause → drain → snapshot → destroy → done`. Crash-safe: snapshot strictly precedes destroy — a crash leaves either everything intact or a durable snapshot + partial teardown, never destruction without capture. Resume: re-running after a crash skips already-completed phases. Multi-host version preflight rejects stale-daemon scenarios (#801). Drain is bounded (60s); `--force-drain` captures stragglers in the snapshot instead of stopping. `--dry-run` prints the snapshot + destroy plan and exits before PAUSE. No WIRE-PROTOCOL changes — orchestrates existing signals/queries. Crash-proof CLI: no static Temporal imports; Temporal is dynamic-imported inside try/catch. (PR #813)
- **Pi model arg on `recruit` (closes #749 via PR #734)** — headless Pi players (`agent: 'pi'`) now accept a `model` recruit arg in `provider/model` format (e.g. `lmstudio/qwen/qwen3.6-27b`), enabling hybrid ensembles where a frontier-model conductor recruits local-model workers. Custom providers (lmstudio, ollama, vllm, user-defined) resolve via Pi's `ModelRegistry` (merges `~/.pi/agent/models.json`) before falling back to the built-in index, matching interactive `pi` resolution order. **Known constraint**: the `model` schema regex is lowercase-only — mixed-case ids (e.g. `lmstudio/Qwen3-27B`) are rejected at schema validation; widening tracked in #773. (PR #734)
- **Command-center / mission-control** — interactive Pi TUI that is simultaneously a live ensemble board and an LLM planning seat. Board: coarse ensemble view via `/v1/events/:ensemble` SSE + fine per-player tail via `/inner` (T3); operator controls (`cue`/`pause`/`play`/`restart`/`destroy`) POST to the daemon write surface. Planner: `ask`/`handoff`/`cue`/`recruit`/`observe_board` tools backed by the daemon HTTP surface. Never claims attachment or registers as a player. Launch: `agent-tempo command-center [ensemble]` (aliases: `cc`, `board`). Loopback auto-token: a localhost daemon grants full trust without `AGENT_TEMPO_HTTP_ADMIN_TOKEN` (closes #736). Requires `@earendil-works/pi-coding-agent`. Closes #700. (PRs #709, #730, #737)
- **Maestro Q&A mailbox + `respond` tool** — operators can ask players questions via correlated `cue` tags (`[Q <questionId>]`); players answer with the `respond` MCP tool, parking the answer on the per-ensemble maestro mailbox (TTL 1h, 20-slot cap). The command-center planner is woken by an `answer` SSE event. (PR #709, command-center P2)
- **`install-pi` prunes stale extension paths on re-run** — `installPiExtensions` now replaces outdated agent-tempo extension entries (e.g. stale version-hashed pnpm paths) instead of accumulating both old and new, preventing Pi from failing to load a missing path after upgrade. Closes #738. (PR #735)
- **Pi extension role-gating** — command-center mode activates only when launched via `agent-tempo command-center`; bare `pi` sessions stay dormant and pristine. Eliminates the phantom-player orphan that appeared when a bare Pi session exited without claiming attachment. Closes #729. (PR #730)
- **Multi-ensemble command-center (closes #790)** — mission-control operators can now list all ensembles (`/ensembles` slash command, `list_ensembles` planner tool) and re-bind to a different ensemble (`/ensemble <name> [--force]`, `bind_ensemble`) without restarting the Pi session. The full operator surface (cue/pause/play/restart/destroy/observe_board) and all planner tools follow the bind. Zero new daemon routes — uses the existing `GET /v1/ensembles` endpoint. `--force` flag bypasses daemon-list validation for temporarily-invisible ensembles (SSE route has its own strongly-consistent fallback). (PR #806)

### Observability — Suspension visibility (#752)

- **`cue`, `broadcast`, `ensemble`, `who_am_i` warn on suspension** — when the sender's outbox is held, the ensemble is paused, or the cue target is paused/held, `cue` now responds **queued — will deliver on resume**, names the suspension axis, and gives the resume verb (`play`, with `release: true` to free held sessions). `broadcast` warns on ensemble + sender suspension. `ensemble` and `who_am_i` prepend `⏸ ENSEMBLE PAUSED` / `⏸ YOUR SESSION IS HELD` banners. Mission-control board tracks and renders `[PAUSED]`/`[HELD]` header markers. Preflight is bounded (1s, soft-fail) and parallel — no added latency window, no breakage if maestro is missing or pre-suspension-era. Closes #752. (PR #755)

### Behavioral changes and breaking notes

- **Pi permission layers removed** — headless Pi players (`agent: 'pi'`) run the full tool surface — including shell — with no approval step, identically to `claude-code-headless` and `opencode`. Removed: the MD-G operator gate (`gate-arm`/`gate-disarm` routes, `GateRegistry`, gate-audit JSONL at `~/.agent-tempo/gate-audit/`, `gateArmed` presence field, mission-control `/arm` + `/gate` commands) and the MD-C `toolAccess` axis (`'restricted'|'standard'|'full'` recruit param and `guardrailPolicy` postures). The `noExtensions: true` supply-chain guard stays. **Wire**: no signal/query/update names changed; optional `guardrailPolicy?`/`toolAccess?` data fields removed from `getMetadata`/`enqueueSpawn` (replay-safe — old in-flight workflows are unaffected). Gate-audit dirs left on disk; safe to delete manually. See `docs/design/pi-streamline-gate-removal-cc.md`. (PR #743)

  > `!` **SemVer reconciliation (#743)**: this removed a feature that only ever shipped in the 1.7.0 betas (never in a stable release). Per SemVer §9, breaking changes within a prerelease line do not affect stable consumers — no stable consumer is broken.

- **`costProfile` defaults to `'local'`** — all cloud-profile gates (T0.1 maestro cadence, T0.4 aggregate demand gate) are off by default. Local profile is byte-identical to pre-1.7.0; no operator action needed on upgrade. To activate: `agent-tempo config set costProfile cloud`, then restart the daemon and terminate/restore `agent-maestro-*` workflows. Full checklist: `docs/ops/sa-diet-migration.md`.
- **T0.2 idle cue latency (SDK adapters)** — with SDK idle backoff enabled, the first cue after a long idle period can take up to 30s (disconnected) or 60s (doorbell-connected) to be noticed by the target. Delivery is durable. Escape hatches: `AGENT_TEMPO_SDK_POLL_MAX_MS=2000` (restores 2s fixed cadence) or `AGENT_TEMPO_SDK_POLL_DOORBELL_MAX_MS=<ms>` (lowers the connected ceiling). Pi players use `IdleBackoff` with the same ceilings — override via `AGENT_TEMPO_PUMP_POLL_MAX_MS` / `AGENT_TEMPO_PUMP_POLL_DOORBELL_MAX_MS`.
- **T0.6 maestro action-counter source tag split** — the `'maestro'` key in `/v1/debug/action-counters` and `[agent-tempo:action-counters]` log lines is no longer emitted. Dashboards and scripts that read the `maestro` line must switch to the three new split keys: `'maestro-ensemble'`, `'maestro-global'`, `'maestro-session'`.
- **SA diet memo fields are v1.7+** — `GitRoot`, `PlayerType`, `IsConductor`, `Part`, and the new `AgentTempoWorkDir`/`AgentTempoAgentType`/`AgentTempoGitBranch` keys are now in workflow memo (not search attributes) for v1.7+ sessions. Pre-v1.7 in-flight workflows use legacy SA reads until they continue-as-new. Hand-written visibility queries on the deprecated attributes match nothing for v1.7+ runs. Old `ClaudeTempo*` / pre-diet SA registrations can be cleaned up; see `docs/ops/sa-diet-migration.md`.

  > `!` **SemVer reconciliation (#757)**: the SA diet uses a dual-read fallback (memo-preferred, SA-fallback) that keeps existing namespaces working; legacy SAs are retained. Marked `!` for wire-surface-sensitivity, NOT a consumer break — the breaking REMOVAL of the fallback is deferred to 2.0 via `upgrade-to-2`.

> **`v1.8-*` patch marker names** — `patched()` markers recorded in running workflow histories (e.g. `v1.8-sa-diet`, `v1.8-t06-chat-gate`) were named during development before the stable version was finalized. They are deliberately NOT renamed: renaming would break replay determinism for any workflow history that recorded them. The `'1.8'` prefix carries no version meaning — treat it as an opaque label.

### Internal: Test harness

- **Per-`withWorker` unique task queues (closes #721)** — every `withWorker*` helper mints a process-unique task queue, structurally eliminating the SlotKey overlap that caused the `session-phase-claim` flake class. Mocha wall-clock dropped 11m → 8m. `createWorkerWithSlotRetry` retained as host-queue backstop (retirement in #772). Serial-`withWorker` invariant now fence-tested. (PR #770)
- **Sticky-queue failover bounded to 1s (closes #777)** — `stickyQueueScheduleToStartTimeout` injected into every test worker (default 10s → 1s), killing the `stages.test.ts` `#181`-class flake where a missed sticky dispatch stalled workflow progress for exactly the test's retry budget. (PR #778)
...
- **Both Pi permission layers removed — headless Pi players now run the FULL tool surface, including shell, with no approval step** (approved direction; design record: `docs/design/pi-streamline-gate-removal-cc.md`). A recruited `agent: 'pi'` player behaves exactly like the other adapters (`claude-code-headless`, `opencode`): every Pi built-in tool and every agent-tempo MCP tool executes without operator approval. Observability is unchanged (mission-control board, coarse SSE, fine `/inner` tail); control is unchanged (`cue`/`pause`/`play`/`restart`/`destroy`/`reset`). Specifically removed:
  ...
  - **MD-C `toolAccess` recruit param** (`'restricted' | 'standard' | 'full'`, phase 3a) and the `src/security/tool-capability.ts` classifier — Bash/shell/exec are no longer blocked or registration-excluded for Pi players. `force: true` is no longer needed (or accepted) for tool access; `force` keeps its other meanings (replace existing session, pre-flight bypass).
