...
<!-- DRAFT for 1.8.0 — version header and date left blank for operator release. All content feature-complete. #811 (pid-file fix) not yet created — add when it lands. -->

### Cost — Temporal Tier-0 programme (#747)

> **Activation**: optimizations are split between `costProfile: 'local'` (default — byte-identical to pre-1.8.0) and `costProfile: 'cloud'` (opt-in; see Behavioral notes). Set `agent-tempo config set costProfile cloud` and restart the daemon and maestros to activate the cloud-profile gates.
...
- **SA diet (T0.5)** — custom Temporal search attributes trimmed 9 → 5, keeping only the five that appear in visibility query expressions. `GitRoot`, `PlayerType`, `IsConductor`, and `Part` migrate to workflow memo (same key names); `AttachmentId` deleted (zero readers). Frees half the 10-Keyword namespace cap, eliminating the recurring "legacy `ClaudeTempo*` SAs block `up`" operator pain point and removing 9 billable SA-upsert sites per session lifecycle. Pre-v1.8 in-flight workflows fall back to legacy SA reads via a dual-read choke point and age out naturally. See `docs/ops/sa-diet-migration.md`. Refs #747. (PR #757)
...
- **`costProfile` defaults to `'local'`** — all cloud-profile gates (T0.1 maestro cadence, T0.4 aggregate demand gate) are off by default. Local profile is byte-identical to pre-1.8.0; no operator action needed on upgrade. To activate: `agent-tempo config set costProfile cloud`, then restart the daemon and terminate/restore `agent-maestro-*` workflows. Full checklist: `docs/ops/sa-diet-migration.md`.
...
- **SA diet memo fields are v1.8+** — `GitRoot`, `PlayerType`, `IsConductor`, `Part`, and the new `AgentTempoWorkDir`/`AgentTempoAgentType`/`AgentTempoGitBranch` keys are now in workflow memo (not search attributes) for v1.8+ sessions. Pre-v1.8 in-flight workflows use legacy SA reads until they continue-as-new. Hand-written visibility queries on the deprecated attributes match nothing for v1.8+ runs. Old `ClaudeTempo*` / pre-diet SA registrations can be cleaned up; see `docs/ops/sa-diet-migration.md`.
