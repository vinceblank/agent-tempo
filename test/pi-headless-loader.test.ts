/**
 * Security regression test for S2 (supply-chain hygiene) —
 * `buildPiResourceLoaderOptions` (src/pi/headless.ts).
 *
 * A daemon-spawned headless player must load ONLY the inline agent-tempo
 * extension — never arbitrary disk/package extensions. Verified against Pi SDK
 * 0.78 source: `DefaultResourceLoader.reload()` loads DISK/package extensions
 * (`~/.pi/agent/extensions/`, `<cwd>/.pi/extensions/`, installed packages)
 * UNLESS `noExtensions: true` — and that flag defaults to `false`. So the
 * loader options MUST set `noExtensions: true` and MUST NOT add
 * `additionalExtensionPaths`, or unreviewed third-party code runs inside a
 * recruited player's process.
 *
