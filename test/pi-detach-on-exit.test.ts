 ...
 * `pub.stop` / `reset.stop` / `wf.detach`, so we mock just those.
 */
function fakeRuntime(opts: { detachRejects?: boolean } = {}): { rt: SeededRuntime; calls: FakeCalls } {
  const calls: FakeCalls = { pubStop: 0, resetStop: 0, detachArgs: [] };
  const rt = {
    pub: { stop: () => { calls.pubStop += 1; } },
    reset: { stop: () => { calls.resetStop += 1; } },
    wf: {
...
describe('detachAllPiRuntimesForExit — graceful exit (H6 #645)', () => {
  ...
  it('stops pub+reset, detaches each with "agent-exited", and a throwing detach neither crashes the exit nor strands the others', async () => {
    ...
    // Every runtime had its publisher + reset poll stopped exactly once.
    expect(a.calls.pubStop, 'A pub.stop').to.equal(1);
    expect(a.calls.resetStop, 'A reset.stop').to.equal(1);
    expect(b.calls.pubStop, 'B pub.stop').to.equal(1);
    expect(b.calls.resetStop, 'B reset.stop').to.equal(1);
